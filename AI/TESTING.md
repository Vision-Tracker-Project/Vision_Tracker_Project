...
게임패드·차량의 하드웨어 없는 테스트: `python -m unittest tests.test_vehicle_control tests.test_gamepad_device tests.test_uart_protocol -v`. `test_vehicle_control`은 호스트 GCC로 실제 STM32 차량 C 코드를 임시 공유 라이브러리로 컴파일합니다. GCC가 없으면 해당 C 테스트만 skip합니다. 세부 범위는 [GAMEPAD.md](GAMEPAD.md)를 참고하세요.
