...
STM32 PWM은 명령각 0∼180도를 500∼2500 us로 변환한다. 초기 PWM은 팬
1500 us, 틸트 800 us이며 각각 프로토콜 명령각 90도와 약 27도에 해당한다.
틸트의 카메라 물리각은 브래킷 장착 오프셋 때문에 약 72도다. Jetson 제어
서비스는 UART 연결 직후 차량 정지 프레임을 전송하고, 이어서 팬 90도
(`AA 01 01 5A 5C 55`)와 틸트 27도(`AA 02 01 1B 1E 55`) 프레임을 한 번의
UART 쓰기로 전송한다. UART 재연결 시에도 동일하게 초기 위치를 다시 맞춘다.
