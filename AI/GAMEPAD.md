...
현재 개발 브랜치 `feat/gamepad-control`에 구현했다. 현재 STM32 기준 코드는 UART 에코뿐이었으며 TIM2 서보 구현은 다른 브랜치에 있다. 해당 브랜치는 읽기 전용으로 확인했다. 이 변경은 PA0/PA1, TIM2, `clock.c`를 수정하지 않는다.
...
# 카메라 GUI와 함께 사용. 동일 옵션이며 UART 소유자는 하나다.
python main.py --gamepad --config gamepad.local.json
```

GUI의 실제 UART도 `--uart 경로`가 필요하다. 과거 `VISION_UART_PORT` 환경 변수만으로 UART를 열던 동작은 명시적 선택 방식으로 변경했다. CLI와 GUI를 같은 UART에 동시에 실행하지 않는다. 영상 작업자가 표시하는 서보 전송 여부는 이제 메일박스 접수를 의미하며 하드웨어 수신 확인이 아니다.
...
`ControlService`만 UART를 열고 쓴다. 카메라는 최신 서보 패킷 한 묶음만 메일박스에 넣는다. UART 쓰기는 50 ms timeout이며 제어 경로에서는 무한 대기 가능성이 있는 flush를 사용하지 않는다. 별도 heartbeat 송신 스레드는 없다. 제어 루프가 멈추면 재전송도 멈춘다. 예외·GUI 종료·SIGINT/SIGTERM은 정지 패킷을 최선의 노력으로 전송하고 포트를 닫는다. 전송 실패·프로세스 강제 종료는 STM32 watchdog이 담당한다. OS/USB 버퍼 지연까지 실시간 보장하는 구조는 아니므로 실제 지연 측정이 필요하다.
...
`Vehicle_Servo_Command(target, angle)`는 현재 weak no-op이다. 기존 브랜치의 TIM2 초기화와 각도 적용 함수를 그대로 유지하고 이 콜백을 strong 정의로 연결한다. `TIM2_Servo_Init()`는 기존 위치에서 호출한다. USART2_IRQHandler와 수신 버퍼는 하나만 유지하고 기존 별도 `Packet_Receive()` 경로와 중복 수신하지 않는다. 서보 콜백에서 UART 블로킹 응답이나 긴 작업을 하지 않는다. SysTick 사용 브랜치와 통합할 때는 기존 tick에 차량 1 ms 처리와 watchdog을 통합해야 한다. 현재 브랜치에서는 서보 하드웨어 구현이 없으므로 서보 구동을 새로 추가하지 않았다.
