...
YOLO는 사람 클래스만 검출하고 BoT-SORT로 단기 ID를 유지합니다. 선택한
사람의 상단 영역에서 YuNet 얼굴을 찾고 SFace 얼굴 ReID를 우선 사용하며,
얼굴을 사용할 수 없으면 OSNet 전신 ReID로 재연결합니다. 얼굴이 검출되면
실제 얼굴 중심을, 검출되지 않으면 전신 박스 상단을 팬·틸트 목표로 사용합니다.
목표 각도가 실제로 변경될 때만 UART 메일박스에 패킷을 제출합니다.
