...
실시간 영상은 JPEG 한 장씩 최신 프레임을 요청하는 방식입니다. WebRTC는 실제 장비에서 약 2초 영상 지연과 높은 RTT가 확인되어 제거했습니다. 화면 상단에는 카메라·YOLO 사람 추적·PNG 저장 기능을, 하단에는 USB 게임패드 입력과 실제 좌우 모터 출력을 표시합니다. 차량 명령은 웹에서 만들지 않습니다.
...
YOLO는 사람 클래스만 검출하고 BoT-SORT와 OSNet 전신 ReID로 선택 대상을
...
- `AI + JPEG 영상`: YOLO·BoT-SORT·OSNet 추적·팬틸트 제어·박스 표시
...
- 게임패드 방향축, 주행 허용, 버튼, 속도 단계, 좌우 모터 출력을 실시간 표시
